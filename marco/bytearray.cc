...
bool ByteArray::isLittleEndian() const {
    return m_endian == MARCO_LITTLE_ENDIAN;
}

void ByteArray::setIsLittleEndian(bool val) {
    if (val) {
        m_endian = MARCO_LITTLE_ENDIAN;
    } else {
        m_endian = MARCO_BIG_ENDIAN;
    }
}
...
bool ByteArray::writeToFile(const std::string& name) const {
    ...
    if (!ofs) {
        MARCO_LOG_ERROR(g_logger) << "writeToFile name=" << name << " error , errno=" << errno
...
bool ByteArray::readFromFile(const std::string& name) {
    ...
    if (!ifs) {
        MARCO_LOG_ERROR(g_logger) << "readFromFile name=" << name << " error, errno=" << errno
                                  << " errstr=" << strerror(errno);
        return false;
    }
