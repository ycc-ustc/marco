...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host, int family,
    ...
    if (!host.empty() && host[0] == '[') {
        ...
        if (endipv6) {
            // TODO(unknown): check out of range
            if (*(endipv6 + 1) == ':') {
                service = endipv6 + 2;
            }
    ...
    if (error) {
        MARCO_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", " << family << ", "
    ...
    while (next) {
        ...
        // MARCO_LOG_INFO(g_logger) << ((sockaddr_in*)next->ai_addr)->sin_addr.s_addr;
        next = next->ai_next;
    }
...
bool Address::GetInterfaceAddresses(
    ...
    if (getifaddrs(&results) != 0) {
        MARCO_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                     " err="
    ...
    try {
        ...
        MARCO_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    if (error) {
        MARCO_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address << ", " << port
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        MARCO_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", " << port
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port) {
    ...
    m_addr.sin_port = byteswapOnLittleEndian(port);  // 网路字节序是大端
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        MARCO_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", " << port
                                  << ") rt=" << result << " errno=" << errno
                                  << " errstr=" << strerror(errno);
        return nullptr;
    }
