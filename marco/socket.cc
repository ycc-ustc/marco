...
Socket::ptr Socket::CreateTCP(marco::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(marco::Address::ptr address) {
...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if (rt) {
        MARCO_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock << " level=" << level
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if (setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        MARCO_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock << " level=" << level
                                  << " option=" << option << " errno=" << errno
                                  << " errstr=" << strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept() {
    ...
    // ::"::" 表示要调用的函数位于全局作用域。通过使用 "::"
    // 作为前缀，可以显式地指定要调用的函数不是类成员函数或命名空间中的函数，而是全局作用域中的函数。
    int newsock = ::accept(m_sock, nullptr, nullptr);
    if (newsock == -1) {
        MARCO_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno=" << errno
                                  << " errstr=" << strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if (!isValid()) {
        newSock();
        if (MARCO_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if (MARCO_UNLIKELY(addr->getFamily() != m_family)) {
        MARCO_LOG_ERROR(g_logger) << "bind sock.family(" << m_family << ") addr.family("
    ...
    if (uaddr) {
        ...
        if (sock->connect(uaddr)) {
            ...
            // marco::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }

    if (::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        MARCO_LOG_ERROR(g_logger) << "bind error errrno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
    getLocalAddress();
    return true;
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if (!m_remoteAddress) {
        MARCO_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if (MARCO_UNLIKELY(addr->getFamily() != m_family)) {
        MARCO_LOG_ERROR(g_logger) << "connect sock.family(" << m_family << ") addr.family("
                                  << addr->getFamily() << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if (timeout_ms == (uint64_t)-1) {
        if (::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            MARCO_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if (!isValid()) {
        MARCO_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if (::listen(m_sock, backlog)) {
        MARCO_LOG_ERROR(g_logger) << "listen error errno=" << errno
                                  << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if (getpeername(m_sock, result->getAddr(), &addrlen)) {
        // MARCO_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
...
Address::ptr Socket::getLocalAddress() {
    ...
    if (getsockname(m_sock, result->getAddr(), &addrlen)) {
        MARCO_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock << " errno=" << errno
                                  << " errstr=" << strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
    if (m_family == AF_UNIX) {
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, marco::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, marco::IOManager::WRITE);
}
...
void Socket::initSock() {
    ...
    if (m_type == SOCK_STREAM) {
        // TCP_NODELAY 是一个套接字选项，用于在 TCP 连接中禁用 Nagle 算法。Nagle
        // 算法通过延迟发送小数据包以提高网络传输效率，但可能会导致一定的延迟。使用 TCP_NODELAY
        // 选项可以禁用 Nagle 算法，从而降低延迟并提高实时性。
...
void Socket::newSock() {
    ...
    if (MARCO_LIKELY(m_sock != -1)) {
        initSock();
    } else {
        MARCO_LOG_ERROR(g_logger) << "socket(" << m_family << ", " << m_type << ", " << m_protocol
                                  << ") errno=" << errno << " errstr=" << strerror(errno);
    }
}
...
//         MARCO_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file(" << cert_file
//                                   << ") error";
//         return false;
//     }
//     if (SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
//         MARCO_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file(" << key_file << ") error";
//         return false;
//     }
//     if (SSL_CTX_check_private_key(m_ctx.get()) != 1) {
//         MARCO_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file=" << cert_file
//                                   << " key_file=" << key_file;
//         return false;
//     }
//     return true;
...
// SSLSocket::ptr SSLSocket::CreateTCP(marco::Address::ptr address) {
