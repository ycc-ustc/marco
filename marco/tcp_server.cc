...
static marco::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout = marco::Config::Lookup(
    "tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2), "tcp server read timeout");

static marco::Logger::ptr g_logger = MARCO_LOG_NAME("system");

TcpServer::TcpServer(marco::IOManager* worker, marco::IOManager* io_worker,
                     marco::IOManager* accept_worker)
    : m_worker(worker),
...
bool TcpServer::bind(marco::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs, std::vector<Address::ptr>& fails,
    ...
    for (auto& i : m_socks) {
        MARCO_LOG_INFO(g_logger) << "type=" << m_type << " name=" << m_name << " ssl=" << m_ssl
...
void TcpServer::startAccept(Socket::ptr sock) {
    while (!m_isStop) {
        ...
        if (client) {
            ...
            MARCO_LOG_ERROR(g_logger) << "accept errno=" << errno << " errstr=" << strerror(errno);
        }
    }
}
