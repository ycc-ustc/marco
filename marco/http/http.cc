...
#define PARSE_PARAM(str, m, flag, trim)                                                            \
    ...
    do {                                                                                           \
        ...
        m.insert(                                                                                  \
            std::make_pair(trim(str.substr(last, key - last)),                                     \
                           marco::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));     \
        if (pos == std::string::npos) {                                                            \
            break;                                                                                 \
        }                                                                                          \
        ++pos;                                                                                     \
    ...
    //         return;
    //     }
    //     PARSE_PARAM(cookie, m_cookies, ';', marco::StringUtil::Trim);
    //     m_parserParamFlag |= 0x4;
    // }

    http::HttpResponse::HttpResponse(uint8_t version, bool close)
    ...
    void HttpResponse::setCookie(const std::string& key, const std::string& val, time_t expired,
        ...
        ss << key << "=" << val;
        if (expired > 0) {
            // ss << ";expires=" << marco::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
        }
        if (!domain.empty()) {
            ss << ";domain=" << domain;
        }
