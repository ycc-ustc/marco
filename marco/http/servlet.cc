...
namespace http {
    ...
    int32_t FunctionServlet::handle(marco::http::HttpRequest::ptr  request,
                                    marco::http::HttpResponse::ptr response,
                                    marco::http::HttpSession::ptr  session) {
        return m_cb(request, response, session);
    }

    ServletDispatch::ServletDispatch() : Servlet("ServletDispatch") {
        m_default.reset(new NotFoundServlet("marco/1.0"));
    }

    int32_t ServletDispatch::handle(marco::http::HttpRequest::ptr  request,
                                    marco::http::HttpResponse::ptr response,
                                    marco::http::HttpSession::ptr  session) {
    ...
    void
    ServletDispatch::listAllServletCreator(std::map<std::string, IServletCreator::ptr>& infos) {
        RWMutexType::ReadLock lock(m_mutex);
        for (auto& i : m_datas) {
    ...
    int32_t NotFoundServlet::handle(marco::http::HttpRequest::ptr  request,
                                    marco::http::HttpResponse::ptr response,
                                    marco::http::HttpSession::ptr  session) {
        response->setStatus(marco::http::HttpStatus::NOT_FOUND);
        response->setHeader("Server", "marco/1.0.0");
