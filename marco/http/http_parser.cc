...
namespace http {
    ...
    static marco::ConfigVar<uint64_t>::ptr g_http_request_buffer_size = marco::Config::Lookup(
        "http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

    static marco::ConfigVar<uint64_t>::ptr g_http_request_max_body_size = marco::Config::Lookup(
        "http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

    static marco::ConfigVar<uint64_t>::ptr g_http_response_buffer_size = marco::Config::Lookup(
        "http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

    static marco::ConfigVar<uint64_t>::ptr g_http_response_max_body_size = marco::Config::Lookup(
    ...
    void on_request_method(void* data, const char* at, size_t length) {
        ...
        if (m == HttpMethod::INVALID_METHOD) {
            MARCO_LOG_WARN(g_logger) << "invalid http request method: " << std::string(at, length);
    ...
    void on_request_fragment(void* data, const char* at, size_t length) {
        MARCO_LOG_INFO(g_logger) << "on_request_fragment:" << std::string(at, length);
        HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
    ...
    void on_request_version(void* data, const char* at, size_t length) {
        ...
        if (strncmp(at, "HTTP/1.1", length) == 0) {
            ...
            MARCO_LOG_WARN(g_logger) << "invalid http request version: " << std::string(at, length);
    ...
    void on_request_http_field(void* data, const char* field, size_t flen, const char* value,
        ...
        HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
        if (flen == 0) {
            MARCO_LOG_WARN(g_logger) << "invalid http request field length == 0";
    ...
    HttpRequestParser::HttpRequestParser() : m_error(0) {
        m_data.reset(new marco::http::HttpRequest);
    ...
    void on_response_http_field(void* data, const char* field, size_t flen, const char* value,
        ...
        HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
        if (flen == 0) {
            MARCO_LOG_WARN(g_logger) << "invalid http response field length == 0";
            // parser->setError(1002);
            return;
        }
        parser->getData()->setHeader(std::string(field, flen), std::string(value, vlen));
    ...
    HttpResponseParser::HttpResponseParser() : m_error(0) {
        m_data.reset(new marco::http::HttpResponse);
